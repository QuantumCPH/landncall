...
<!--<tr bgcolor="#CCCCCC" class="receipt_header"> 
  ...
  <tr> 
    <td colspan="4" class="payer_summary">
	  ...
	  ?>
      <br /><br />
          <?php echo __('US Mobile Number') ?>   <?php   $us = new Criteria();
            $us->add(UsNumberPeer::CUSTOMER_ID, $customer->getId());
             $usnumber = UsNumberPeer::doSelectOne($us);
              echo   $usnumber->getUsMobileNumber()

             ?><br/>
                <?php echo __('Resenumer Number') ?>   <?php    $un = new Criteria();
            $un->add(SeVoipNumberPeer::CUSTOMER_ID,$customer->getId() );
  ...
  <tr> 
    ...
    <?php if ($order->getIsFirstOrder())
    {
        ...
        if($transaction->getDescription()=="Registrering inkl. taletid"){
          echo "<br />[US Package inklusive pott]";
        }else{
