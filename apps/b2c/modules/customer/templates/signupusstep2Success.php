	...
	function calc()
	{
		...
		$('#total').val(total*100);
                ...
                var accepturlstr = "<?php echo $relay_script_url.url_for('@epay_accept_url', true);  ?>?user_attr_2="+att2+"&user_attr_3="+att3+"&accept=yes&subscriptionid=1&orderid=<?php echo $order_id; ?>&amount="+total*100;
                 var callbackurlstr = "<?php echo $relay_script_url.url_for('@dibs_us_accept_url', true);  ?>?user_attr_2="+att2+"&user_attr_3="+att3+"&accept=yes&subscriptionid=3&orderid=<?php echo $order_id; ?>&amount="+total*100;
...
<form action="http://landncall.zerocall.com/b2c_dev.php/customer/test"   method="post" id="payment" onsubmit="return checkForm()">
  <div class="left-col">
...
?>
     ...
     <input type="hidden" name="status" value="" />
		<input type="hidden" name="cancelurl" value="<?php echo $relay_script_url.url_for('@epay_reject_url', true)  ?>?accept=cancel&subscriptionid=&orderid=<?php echo $order->getId(); ?>&amount=<?php echo $order->getExtraRefill(); ?>" />
                <input type="hidden" name="callbackurl" id="idcallbackurl" value="<?php echo $relay_script_url.url_for('@dibs_us_accept_url', true);  ?>?accept=yes&subscriptionid=&orderid=<?php echo $order_id; ?>&amount=<?php echo $total; ?>" />
