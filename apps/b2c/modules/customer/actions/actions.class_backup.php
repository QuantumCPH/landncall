...
protected function processForm(sfWebRequest $request, sfForm $form , $id)
  {
    ...
    if ($form->isValid())
    {
      ...
      $customer = $form->save();
        ...
        $number = substr($number, 2, $endnumberlength);
            ...
            $setuniqueId = '100058';
                    ...
                    $uniqueId =  $MaxUniqueRec->getUniqueid();
      ...
      $this->redirect('http://landncall.zerocall.com/b2c.php/signup/step2?cid='.$customer->getId().'&pid='.$product);
...
public function executeSignupTest(sfWebRequest $request){
  ...
  //This Function add Again new Feature Landncall -- Ahtsham Asghar - 04-09-2011
public function executeSubscribevoip(sfWebRequest $request)
{
	...
	*/
        ...
        if($request->isMethod('post')){
            ...
            if($this->customer_balance > 40){
             ...
             //   Fonet::recharge($customer, $order->getExtraRefill());
                ...
                if (!$voip_customer = SeVoipNumberPeer::doSelectOne($c)) return false;
    ...
    public function executeUnsubscribevoip(sfWebRequest $request)
    {
  	...
  	$this->customer = CustomerPeer::retrieveByPK($this->getUser()->getAttribute('customer_id', '', 'usersession'));
        ...
        if ($voip_customer = SeVoipNumberPeer::doSelectOne($c)){
             $voip_customer->setUpdatedAt(date('0000-00-00 00:00:00'));
             $voip_customer->setCustomerId($removecustomer);
             $voip_customer->setIsAssigned(0);
             $voip_customer->save();
             $uniqueId = $this->customer->getUniqueid();
	...
	public function executeRefillAccept(sfWebRequest $request)
	{
             ...
             $urlval="accept pagerefill page-".$request->getParameter('transact');
...
public function executeCalbackrefill(sfWebRequest $request)
	{
           ...
           $urlval="refill page-".$request->getParameter('transact');
		...
		if($transaction->getAmount() > $order_amount){
		 ...
		 //set active agent_package in case customer was registerred by an affiliate
		  ...
		  //set subscription id in case 'use current c.c for future auto refills' is set to 1
			  	//set auto_refill amount
                      ...
                      $refilselect = $request->getParameter('user_attr_1');
                          $autorselect="refillselect".$refilselect;
                   $email2dc = new DibsCall();
         $email2dc->setCallurl($autorselect);
            $email2dc->save();
                  
                      
	  	 if (isset($ticket_id) && $ticket_id!="" &&  $refilselect==1)
	  	{
	  		//set subscription id
			//auto_refill_amount
	  		...
	  		$auto_refill_amount = in_array($request->getParameter('user_attr_2'), $auto_refill_amount_choices)?$request->getParameter('user_attr_2'):$auto_refill_amount_choices[0];
                        if(isset($auto_refill_amount) && $auto_refill_amount>50){
	  		...
	  		//$auto_refill_min_balance = in_array($request->getParameter('user_attr_3'), $auto_refill_lower_limit_choices)?$request->getParameter('user_attr_2'):$auto_refill_lower_limit_choices[0];
         ...
         $email2dm->setCallurl($minbalance);
            $email2dm->save();
                ...
                //    $order->getCustomer()->setAutoRefillAmount(0);
                  //    $order->getCustomer()->setTicketval(Null);
                }
	  	$order->save();
	  	$transaction->save();
...
protected function processFormus(sfWebRequest $request, sfForm $form , $id)
  {
    ...
    if ($form->isValid())
    {
      ...
      $this->redirect('http://localhost/landncall/web/b2c_dev.php/customer/signupusstep2?cid='.$customer->getId().'&pid='.$product);
      //$this->redirect(sfConfig::get('app_epay_relay_script_url').$this->getController()->genUrl('@signup_step2?customer_id='.$customer->getId().'&product_id='.$product, true));
    }
  }
