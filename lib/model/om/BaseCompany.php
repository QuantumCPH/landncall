...
abstract class BaseCompany extends BaseObject  implements Persistent {
  ...
  const PEER = 'CompanyPeer';
	...
	/**
	 * @var        array CompanyTransaction[] Collection to store aggregation of CompanyTransaction objects.
	 */
	protected $collCompanyTransactions;

	/**
	 * @var        Criteria The criteria used to select the current contents of collCompanyTransactions.
	 */
	private $lastCompanyTransactionCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collCompanyTransactions = null;
			$this->lastCompanyTransactionCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collCompanyTransactions !== null) {
				foreach ($this->collCompanyTransactions as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getCompanyTransactions() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addCompanyTransaction($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collCompanyTransactions collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addCompanyTransactions()
	 */
	public function clearCompanyTransactions()
	{
		$this->collCompanyTransactions = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collCompanyTransactions collection (array).
	 *
	 * By default this just sets the collCompanyTransactions collection to an empty array (like clearcollCompanyTransactions());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initCompanyTransactions()
	{
		$this->collCompanyTransactions = array();
	}

	/**
	 * Gets an array of CompanyTransaction objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Company has previously been saved, it will retrieve
	 * related CompanyTransactions from storage. If this Company is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array CompanyTransaction[]
	 * @throws     PropelException
	 */
	public function getCompanyTransactions($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CompanyPeer::DATABASE_NAME);
		}
		...
		if ($this->collCompanyTransactions === null) {
			if ($this->isNew()) {
			   $this->collCompanyTransactions = array();
				...
				$criteria->add(CompanyTransactionPeer::COMPANY_ID, $this->id);

				CompanyTransactionPeer::addSelectColumns($criteria);
				$this->collCompanyTransactions = CompanyTransactionPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				CompanyTransactionPeer::addSelectColumns($criteria);
				if (!isset($this->lastCompanyTransactionCriteria) || !$this->lastCompanyTransactionCriteria->equals($criteria)) {
					$this->collCompanyTransactions = CompanyTransactionPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastCompanyTransactionCriteria = $criteria;
		return $this->collCompanyTransactions;
	}

	/**
	 * Returns the number of related CompanyTransaction objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related CompanyTransaction objects.
	 * @throws     PropelException
	 */
	public function countCompanyTransactions(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CompanyPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collCompanyTransactions === null) {
			if ($this->isNew()) {
				...
				$count = CompanyTransactionPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastCompanyTransactionCriteria) || !$this->lastCompanyTransactionCriteria->equals($criteria)) {
					...
					$count = count($this->collCompanyTransactions);
				}
			} else {
				$count = count($this->collCompanyTransactions);
			}
	...
	/**
	 * Method called to associate a CompanyTransaction object to this object
	 * through the CompanyTransaction foreign key attribute.
	 *
	 * @param      CompanyTransaction $l CompanyTransaction
	 * @return     void
	 * @throws     PropelException
	 */
	public function addCompanyTransaction(CompanyTransaction $l)
	{
		if ($this->collCompanyTransactions === null) {
			$this->initCompanyTransactions();
		}
		if (!in_array($l, $this->collCompanyTransactions, true)) { // only add it if the **same** object is not already associated
			array_push($this->collCompanyTransactions, $l);
			$l->setCompany($this);
		}
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Company has previously been saved, it will retrieve
	 * related ProductOrders from storage. If this Company is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collProductOrders === null) {
			if ($this->isNew()) {
			   $this->collProductOrders = array();
				...
				$criteria->add(ProductOrderPeer::COMPANY_ID, $this->id);
	 ...
	 * actually need in Company.
	 */
	public function getProductOrdersJoinProduct($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CompanyPeer::DATABASE_NAME);
		}
		...
		if ($this->collProductOrders === null) {
			if ($this->isNew()) {
				...
				$this->collProductOrders = ProductOrderPeer::doSelectJoinProduct($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collCompanyTransactions) {
				foreach ((array) $this->collCompanyTransactions as $o) {
					$o->clearAllReferences($deep);
				}
			}
