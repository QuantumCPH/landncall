  ...
  const PEER = 'CompanyPeer';
	...
	public function setPackageId($v)
	{
		...
		if ($this->package_id !== $v) {
			$this->package_id = $v;
			$this->modifiedColumns[] = CompanyPeer::PACKAGE_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->vat_no = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			...
			$this->usage_discount_pc = ($row[$startcol + 26] !== null) ? (double) $row[$startcol + 26] : null;
			$this->registration_date = ($row[$startcol + 27] !== null) ? (string) $row[$startcol + 27] : null;
			...
			$this->file_path = ($row[$startcol + 30] !== null) ? (string) $row[$startcol + 30] : null;
			$this->resetModified();
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[26] => $this->getUsageDiscountPc(),
			$keys[27] => $this->getRegistrationDate(),
			$keys[28] => $this->getCreatedAt(),
			$keys[29] => $this->getUpdatedAt(),
			$keys[30] => $this->getFilePath(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[26], $arr)) $this->setUsageDiscountPc($arr[$keys[26]]);
		if (array_key_exists($keys[27], $arr)) $this->setRegistrationDate($arr[$keys[27]]);
		...
		if (array_key_exists($keys[30], $arr)) $this->setFilePath($arr[$keys[30]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CompanyPeer::PACKAGE_ID)) $criteria->add(CompanyPeer::PACKAGE_ID, $this->package_id);
	...
	public function getPackage(PropelPDO $con = null)
	{
		if ($this->aPackage === null && ($this->package_id !== null)) {
			$c = new Criteria(PackagePeer::DATABASE_NAME);
			$c->add(PackagePeer::ID, $this->package_id);
			$this->aPackage = PackagePeer::doSelectOne($c, $con);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aPackage->addCompanys($this);
			 */
		}
