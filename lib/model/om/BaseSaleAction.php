...
/**
 * Base class that represents a row from the 'sale_action' table.
 *
 * 
 *
 ...
 */
abstract class BaseSaleAction extends BaseObject  implements Persistent {


  const PEER = 'SaleActionPeer';
	...
	/**
	 * Initializes internal state of BaseSaleAction object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     SaleAction The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = SaleActionPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = SaleActionPeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = SaleActionPeer::NUM_COLUMNS - SaleActionPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating SaleAction object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = SaleActionPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseSaleAction:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			SaleActionPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseSaleAction:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseSaleAction:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseSaleAction:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			SaleActionPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = SaleActionPeer::doInsert($this, $con);
					...
					$affectedRows += SaleActionPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = SaleActionPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = SaleActionPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = SaleActionPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
		);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SaleActionPeer::ID)) $criteria->add(SaleActionPeer::ID, $this->id);
		if ($this->isColumnModified(SaleActionPeer::NAME)) $criteria->add(SaleActionPeer::NAME, $this->name);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(SaleActionPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of SaleAction (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     SaleAction Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     SaleActionPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new SaleActionPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this SaleAction has previously been saved, it will retrieve
	 * related SaleActivitys from storage. If this SaleAction is new, it will return
		...
		if ($this->collSaleActivitys === null) {
			if ($this->isNew()) {
			   $this->collSaleActivitys = array();
				...
				$criteria->add(SaleActivityPeer::SALE_ACTION_ID, $this->id);
	 ...
	 * Otherwise if this SaleAction is new, it will return
	 * an empty collection; or if this SaleAction has previously
	 * been saved, it will retrieve related SaleActivitys from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in SaleAction.
	 */
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseSaleAction:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseSaleAction::%s', $method));
    }
