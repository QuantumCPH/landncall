...
/**
 * Base class that represents a row from the 'company_size' table.
 *
 * 
 *
 ...
 */
abstract class BaseCompanySize extends BaseObject  implements Persistent {


  const PEER = 'CompanySizePeer';
	...
	/**
	 * Initializes internal state of BaseCompanySize object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     CompanySize The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = CompanySizePeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = CompanySizePeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = CompanySizePeer::NUM_COLUMNS - CompanySizePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating CompanySize object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = CompanySizePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseCompanySize:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			CompanySizePeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseCompanySize:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseCompanySize:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseCompanySize:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			CompanySizePeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = CompanySizePeer::doInsert($this, $con);
					...
					$affectedRows += CompanySizePeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = CompanySizePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = CompanySizePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = CompanySizePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
		);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CompanySizePeer::ID)) $criteria->add(CompanySizePeer::ID, $this->id);
		if ($this->isColumnModified(CompanySizePeer::NAME)) $criteria->add(CompanySizePeer::NAME, $this->name);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(CompanySizePeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of CompanySize (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     CompanySize Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     CompanySizePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new CompanySizePeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this CompanySize has previously been saved, it will retrieve
	 * related Companys from storage. If this CompanySize is new, it will return
		...
		if ($this->collCompanys === null) {
			if ($this->isNew()) {
			   $this->collCompanys = array();
				...
				$criteria->add(CompanyPeer::COMPANY_SIZE_ID, $this->id);
	 ...
	 * Otherwise if this CompanySize is new, it will return
	 * an empty collection; or if this CompanySize has previously
	 * been saved, it will retrieve related Companys from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in CompanySize.
	 */
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseCompanySize:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseCompanySize::%s', $method));
    }
