...
abstract class BaseStatus extends BaseObject  implements Persistent {
  ...
  const PEER = 'StatusPeer';
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Status has previously been saved, it will retrieve
	 * related PostalChargess from storage. If this Status is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collPostalChargess === null) {
			if ($this->isNew()) {
			   $this->collPostalChargess = array();
				...
				$criteria->add(PostalChargesPeer::STATUS, $this->id);
	 ...
	 * actually need in Status.
	 */
	public function getPostalChargessJoinEnableCountry($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(StatusPeer::DATABASE_NAME);
		}
		...
		if ($this->collPostalChargess === null) {
			if ($this->isNew()) {
				...
				$this->collPostalChargess = PostalChargesPeer::doSelectJoinEnableCountry($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
