...
abstract class BaseEnableCountry extends BaseObject  implements Persistent {
  ...
  const PEER = 'EnableCountryPeer';
	...
	/**
	 * @var        array PostalCharges[] Collection to store aggregation of PostalCharges objects.
	 */
	protected $collPostalChargess;

	/**
	 * @var        Criteria The criteria used to select the current contents of collPostalChargess.
	 */
	private $lastPostalChargesCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collPostalChargess = null;
			$this->lastPostalChargesCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collPostalChargess !== null) {
				foreach ($this->collPostalChargess as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getPostalChargess() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addPostalCharges($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collPostalChargess collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addPostalChargess()
	 */
	public function clearPostalChargess()
	{
		$this->collPostalChargess = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collPostalChargess collection (array).
	 *
	 * By default this just sets the collPostalChargess collection to an empty array (like clearcollPostalChargess());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initPostalChargess()
	{
		$this->collPostalChargess = array();
	}

	/**
	 * Gets an array of PostalCharges objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this EnableCountry has previously been saved, it will retrieve
	 * related PostalChargess from storage. If this EnableCountry is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 ...
	 * @throws     PropelException
	 */
	public function getPostalChargess($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(EnableCountryPeer::DATABASE_NAME);
		}
		...
		if ($this->collPostalChargess === null) {
			if ($this->isNew()) {
			   $this->collPostalChargess = array();
				...
				$criteria->add(PostalChargesPeer::COUNTRY, $this->id);

				PostalChargesPeer::addSelectColumns($criteria);
				$this->collPostalChargess = PostalChargesPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				PostalChargesPeer::addSelectColumns($criteria);
				if (!isset($this->lastPostalChargesCriteria) || !$this->lastPostalChargesCriteria->equals($criteria)) {
					$this->collPostalChargess = PostalChargesPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastPostalChargesCriteria = $criteria;
		return $this->collPostalChargess;
	}

	/**
	 * Returns the number of related PostalCharges objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related PostalCharges objects.
	 * @throws     PropelException
	 */
	public function countPostalChargess(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(EnableCountryPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collPostalChargess === null) {
			if ($this->isNew()) {
				...
				$count = PostalChargesPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastPostalChargesCriteria) || !$this->lastPostalChargesCriteria->equals($criteria)) {
					...
					$count = count($this->collPostalChargess);
				}
			} else {
				$count = count($this->collPostalChargess);
			}
	...
	/**
	 * Method called to associate a PostalCharges object to this object
	 * through the PostalCharges foreign key attribute.
	 *
	 * @param      PostalCharges $l PostalCharges
	 * @return     void
	 * @throws     PropelException
	 */
	public function addPostalCharges(PostalCharges $l)
	{
		if ($this->collPostalChargess === null) {
			$this->initPostalChargess();
		}
		if (!in_array($l, $this->collPostalChargess, true)) { // only add it if the **same** object is not already associated
			array_push($this->collPostalChargess, $l);
			$l->setEnableCountry($this);
		}
	}
	 ...
	 * Otherwise if this EnableCountry is new, it will return
	 * an empty collection; or if this EnableCountry has previously
	 * been saved, it will retrieve related PostalChargess from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in EnableCountry.
	 */
	public function getPostalChargessJoinStatusRelatedByStatus($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(EnableCountryPeer::DATABASE_NAME);
		}
		...
		if ($this->collPostalChargess === null) {
			if ($this->isNew()) {
				...
				$this->collPostalChargess = PostalChargesPeer::doSelectJoinStatusRelatedByStatus($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collPostalChargess) {
				foreach ((array) $this->collPostalChargess as $o) {
					$o->clearAllReferences($deep);
				}
			}
